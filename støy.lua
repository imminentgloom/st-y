...
function init()
	...
	f_list = {'2.8k', '1.5k', '777', '411', '218', '115', '61', '29'}
...
-- light up all rows with values rounded to nearest 16th
...
g.key = function(x, y, z)
	if z == 1 then                                                			-- only act when keys are pressed
		if k1_held == false then
			...
			word = p_list[p_pos]
			remember = word
			if x == 1 then g_val = 0 else g_val = x / 16 end				-- x possition sets the value, plays better if first button is 0
			params:set_raw(word, g_val)
			p_val = params:get_raw(word)
		elseif k1_held == true then
			f_pos = y
			word = f_list[f_pos]
			if x == 1 then f_val = 0.01 else f_val = x / 16 end				-- x possition sets the value, plays better if first button is ~0
